...
/**
 * Value stack — verbatim from the live page, forked unchanged from
 * app/projects/oos/components/OfferStack.tsx. No morph here (that's
 * oos_v2-only) — this renders as a standalone static section, same as the
 * live page.
 */
...
const ROWS: { item: string; sub?: string; value: number }[] = [
        ...
        <div className="oh-card mt-10 overflow-hidden rounded-2xl border border-[#b7c6c2]/15 bg-[#0f1410] shadow-2xl">
          <ul>
            {ROWS.map((row, i) => (
              ...
              >
                <span>
                  <span className="block text-sm font-medium text-white/90 sm:text-base">{row.item}</span>
                  {row.sub && <span className="mt-0.5 block text-xs text-white/45">{row.sub}</span>}
                </span>
          ...
          <div className="border-t-2 border-dashed border-[#b7c6c2]/20 bg-[#171e19] px-5 py-8 text-center sm:px-8">
            ...
            <p className="oh-display mt-3 text-5xl text-[#ffe17c] sm:text-6xl">Today: {PRICE}</p>
            <p className="mt-2 text-sm font-medium text-white/70">$0 setup · cancel anytime</p>
