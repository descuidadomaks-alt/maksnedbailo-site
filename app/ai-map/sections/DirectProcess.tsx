...
export default function DirectProcess({ d }: { d: DirectPageDict }) {
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="hidden md:block" data-reveal>
          ...
          <div className="grid grid-cols-3 gap-5">
            {d.process.steps.map((step, i) => (
              <div key={step.num} data-reveal={`d${i}`} className="rounded-2xl border border-white/[0.06] bg-white/[0.014] p-7 flex flex-col gap-3 hover:border-accent/18 hover:bg-white/[0.024] hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(0,0,0,0.35)] transition-all duration-300">
                ...
                {step.trust && <p className="font-sora font-light text-fg/28 mt-1 italic" style={{ fontSize: "12px" }}>{step.trust}</p>}
              </div>
            ))}
          </div>
        </div>
          ...
          {d.process.steps.map((step, i) => (
            <div key={step.num} data-reveal={`d${i}`} className="flex gap-4">
              ...
              <div className="flex-1 rounded-2xl border border-white/[0.06] bg-white/[0.014] p-6 flex flex-col gap-3 mb-4 transition-all duration-300">
                ...
                <p className="font-sora font-light text-fg/55 leading-[1.75]" style={{ fontSize: "14px" }}>{step.body}</p>
                {step.trust && <p className="font-sora font-light text-fg/28 italic" style={{ fontSize: "12px" }}>{step.trust}</p>}
              </div>
            </div>
          ))}
        </div>
