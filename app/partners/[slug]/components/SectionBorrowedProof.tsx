      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {CASES.map((c, i) => {
            ...
            const cardClass = `group rounded-2xl border border-white/[0.06] bg-white/[0.018] p-7 flex flex-col gap-4 transition-all duration-300 ${
              isInProgress
                ? "opacity-70"
            ...
            return isInProgress ? (
              <div key={i} data-reveal={`d${i}`} className={cardClass}>
                {/* Title + badge in flex row — badge never overlaps title */}
                <div className="flex items-start justify-between gap-3">
                  ...
                  >
                    ...
                    <span className="font-sora font-semibold whitespace-nowrap" style={{ fontSize: "9px", letterSpacing: "1.5px", color: "rgba(250,204,21,0.85)" }}>
                      IN PROGRESS
                    </span>
                  </div>
                </div>
              ...
              <Link
                ...
                className={cardClass}
              >
                {/* Title + LIVE badge in flex row — no absolute positioning */}
                <div className="flex items-start justify-between gap-3">
                  <h3 className="font-playfair font-normal text-fg" style={{ fontSize: "clamp(17px, 1.5vw, 20px)", lineHeight: 1.25 }}>
                    {c.name}
                  </h3>
                  <div className="shrink-0 flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-fg/25 group-hover:text-fg/50 transition-colors duration-200">
                      <ArrowIcon />
                    </span>
                  </div>
                </div>
