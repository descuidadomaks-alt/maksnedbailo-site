...
export default function SectionShortCTA({
  ...
  const messengerUrl = isTelegram
    ? `https://t.me/${config.messenger.handle}`
  ...
  return (
    <section className="section-divider relative overflow-hidden py-24 md:py-36">
      ...
      <div className="relative max-w-lg mx-auto px-6 text-center">
        ...
        <p
          data-reveal
          className="font-playfair text-fg/45 mb-12"
          style={{ fontSize: "clamp(17px, 1.8vw, 22px)", lineHeight: 1.4, fontStyle: "italic", letterSpacing: "-0.012em" }}
        >
          {d.cta.sub}
        </p>
        ...
        <div data-reveal="d1" className="flex flex-col items-center gap-4">
          ...
          <a
            ...
            onClick={() => track("cta_book_click", { slug: config.slug, location: "final_short" })}
          >
            {d.cta.primaryCta}
          ...
          <a
            ...
            onClick={() =>
              track(isTelegram ? "telegram_click" : "whatsapp_click", {
                slug: config.slug,
                location: "final_short",
              })
            }
          >
            {isTelegram ? <TelegramIcon /> : <WAIcon />}
            {d.cta.messengerLabel(config.messenger.channel)}
          </a>
        </div>
