/**
 * NavNew — identical to Nav, with one addition:
 *   "AI Map" link between Blog and the language switcher.
 * Used on /new (and exported for /ai-map layout).
 * The live Nav is NOT modified.
 */
"use client";
...
/** AI Map label — bilingual */
const AI_MAP_LABEL = { en: "AI Map", es: "Mapa IA" };
    ...
    >
      <div className="max-w-6xl mx-auto px-3 sm:px-4 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 pointer-events-auto">
          ...
          >
            <MapIcon />
            <span className="hidden xs:inline">{t(AI_MAP_LABEL, lang)}</span>
          </Link>
          ...
          <div className="flex items-center gap-0.5 p-1" style={GLASS}>
            <button
              ...
              className={`min-w-[34px] xs:min-w-[38px] min-h-[32px] px-2 xs:px-2.5 py-1.5 rounded-full text-[12px] font-sora font-semibold transition-all duration-200 ${
                lang === "en" ? "bg-accent text-bg" : "text-fg/45 hover:text-fg/75"
              }`}
            ...
            <button
              onClick={() => setLang("es")}
              className={`min-w-[34px] xs:min-w-[38px] min-h-[32px] px-2 xs:px-2.5 py-1.5 rounded-full text-[12px] font-sora font-semibold transition-all duration-200 ${
                lang === "es" ? "bg-accent text-bg" : "text-fg/45 hover:text-fg/75"
              }`}
